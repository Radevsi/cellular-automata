...
class CADiff(nn.Module):
    def __init__(self, c_in, c_noise=3,
        ...
        conv3 = nn.Conv2d(in_channels=self.hidden_size, out_channels=self.c_in, kernel_size=1)
    ...
    def sample(self, model, n):
        ...
        with torch.no_grad():            
            ...
            for i in tqdm(reversed(range(1, self.noise_steps)), position=0):
                ...
                iter_n = np.random.randint(self.lower, self.upper, dtype=np.int32)
...
def get_cifar(download=False, img_size=64, batch_size=8):
    transforms = torchvision.transforms.Compose([
        torchvision.transforms.RandomResizedCrop(img_size, scale=(0.8, 1.0)),
        torchvision.transforms.ToTensor(),
...
######################## Model Instantiations
    ...
    diffusion = Diffusion(c_in=channel_n, c_noise=c_noise, noise_steps=500, img_size=64)

    
    start_time = time.time() # time training run    
    
    run_name = 'CADiff_massive500_CIFAR64'
    train_diff = TrainDiff(ca_diff, diffusion, dataloader=dataloader_cifar, c_in=channel_n,
                           c_noise=c_noise, run_name=run_name)

    losses = train_diff.train(epochs=501)            
